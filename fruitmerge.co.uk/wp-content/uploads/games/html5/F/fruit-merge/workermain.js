...
async function LoadScripts(t) {
  if (1 === t.length) {
    const e = t[0];
    await import((IsRelativeURL(e) ? "./" : "") + e);
  } else {
    const e = t
    ...
    try {
      ...
      for (const e of t) await import((IsRelativeURL(e) ? "./" : "") + e);
    }
  }
}
async function InitRuntime(t) {
  ...
  const i = t["runtimeScriptList"].map((t) =>
    new URL(t, location.href).toString()
  );
  try {
    await LoadScripts([...i]);
  } catch (t) {
  ...
  const a = t["projectMainScriptPath"],
    o = t["javaScriptInEventsPath"],
    s = t["typeScriptInEventsPath"],
    c = new URL(a, location.href).toString(),
  ...
  if (a)
    try {
      if (
        (await LoadScripts([c]),
        "preview" === r && !globalThis.C3_ProjectMainScriptOK)
      )
        throw new Error("main script did not run to completion");
    } catch (t) {
      console.error("Error loading project main script: ", t);
      const r = `Failed to load the project main script (${a}). Check all your JavaScript code has valid syntax, all imports are written correctly, and that an exception was not thrown running the script. Press F12 and check the console for error details.`;
  ...
  if (o)
    try {
      if (
        (await LoadScripts([l]),
        "preview" === r && !globalThis.C3.JavaScriptInEvents)
      )
  ...
  if (s)
    try {
      if (
        (await LoadScripts([p]),
        "preview" === r && !globalThis.C3.TypeScriptInEvents)
      )
